...
class RectItemInserter(ItemInserter):
    ...
    def mousePressEvent(self, event, image_item):
        ...
        self._text_item = QGraphicsTextItem(self._item)
        self._text_item.setDefaultTextColor(self.pen())
    ...
    def mouseMoveEvent(self, event, image_item):
        if self._aiming:
            ...
            if self._item is not None:
                ...
                pos = self._init_pos
                self._text_item.setPos(pos.x(), pos.y())
                self._text_item.setHtml(" ")
                self._text_item.setHtml("W:%d, H:%d" % (rect.width(), rect.height()))
    ...
    def abort(self):
        if self._helpLines is not None and self._helpLines.scene == self._scene:
            self._scene.removeItem(self._helpLines)
            self._helpLines = None

        if self._item is not None and self._item.scene == self._scene:
            self._scene.removeItem(self._item)
            self._item = None
            self._init_pos = None
...
class IDRectItemInserter(ItemInserter):

    def __init__(self, labeltool, scene, default_properties=None, max_id = 0,
                 prefix="", commit=True):
        ItemInserter.__init__(self, labeltool, scene, default_properties,
                              prefix, commit)
        self._aiming = True
        ...
        self._init_pos = None
        self.max_id = max_id
        self._label_class = str(scene._label_class)
        self._shorted_label_class = self._label_class[0]

        if self._label_class == 'Pedestrian':
            self.extra_key = 'type'
            self.default_value = 'pedestrian'
        elif self._label_class == 'Vehicle':
            self.extra_key = 'scale'
            self.default_value = 'small'
    ...
    def mouseReleaseEvent(self, event, image_item):
        if self._item is not None:
            ...
            img_width = self._scene._labeltool._img_width
            img_height = self._scene._labeltool._img_height
               ...
               x + w > 0 and y + h > 0:
                ...
                if x < img_width - w/3. and y < img_height - h/3. and \
                   x + w > w/3. and y + h > h/3.:
                    ...
                    self.max_id += 1
                    self._ann.update({self._prefix + 'ID': self.max_id,
                                    ...
                                    self._prefix + 'SV': 0,
                                    self._prefix + 'CM': 0,
                                    self._prefix + 'class': self._label_class,
                                    self._prefix + self.extra_key: self.default_value,
                                    })
                    self._ann.update(self._default_properties)
    ...
    def insertRect(self, anno, image_item):
        if anno is not None:
            if self._item is None:
                self._item = QGraphicsRectItem(QRectF(anno['x'], anno['y'], anno['width'], anno['height']))
            else:
                self._item.setRect(anno['x'], anno['y'], anno['width'], anno['height'])

            if self._item.rect().width() > 1 and \
               self._item.rect().height() > 1:
                self._ann.update({self._prefix + 'ID': anno['ID'],
                                  self._prefix + 'x': anno['x'],
                                  self._prefix + 'y': anno['y'],
                                  self._prefix + 'width': anno['width'],
                                  self._prefix + 'height': anno['height'],
                                  self._prefix + 'Occ': anno['Occ'],
                                  self._prefix + 'IV': anno['IV'],
                                  self._prefix + 'FM': anno['FM'],
                                  self._prefix + 'SV': anno['SV'],
                                  self._prefix + 'CM': anno['CM'],
                                  self._prefix + 'class': anno['class'],
                                  self._prefix + self.extra_key: anno[self.extra_key],
                                  })
                self._ann.update(self._default_properties)
