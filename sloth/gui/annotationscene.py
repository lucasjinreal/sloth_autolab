...
class AnnotationScene(QGraphicsScene):
    ...
    def setCurrentImage(self, current_image, keepAnnos=False):
        """
        ...
        if current_image == self._image_item:
            ...
            if self._image_item is not None:
                last_anns = self._image_item.getAnnotations()['annotations']
                confirmed = self._image_item._confirmed
            else:
                confirmed = False
            ...
            if keepAnnos and confirmed:
                cur_annos = current_image.getAnnotations()['annotations']
                # self.onInsertionModeStarted(label_class = 'Vehicle')

                if cur_annos is not None and len(cur_annos) == 0 and last_anns is not None and len(last_anns) > 0:
                    for ann in last_anns:
                        self._image_item.addAnnotation(ann)
                        # self._inserter.insertRect(ann, self._image_item)
                elif cur_annos is not None and len(cur_annos) > 0 and last_anns is not None and len(last_anns) > 0:
                    for ann in last_anns:
                        flag = True
                        for ann_cur in cur_annos:
                            if ann['ID'] == int(ann_cur['ID']):
                                flag = False
                                break
                        if flag:
                            self._image_item.addAnnotation(ann)
                            ...
                            # self.onInserterFinished()

            self.insertItems(0, len(self._image_item.children()) - 1)
            self.update()

    def updateAnnotations(self, anno):
        x, y, w, h = anno['x'], anno['y'], anno['width'], anno['height']
        img_width = self._labeltool._img_width
        img_height = self._labeltool._img_height

        if w > 1 and h > 1 and \
                        x < img_width and y < img_height and \
                                x + w > 0 and y + img_height > 0:

            if x < img_width - w / 3. and y < img_height - h / 3. and \
                                    x + w > w / 3. and y + h > h / 3.:
                # self._image_item.addAnnotation(anno)
                # self.insertItems(0, len(self._image_item.children())-1)
                self._inserter.insertRect(anno, self._image_item)
                self.update()
    ...
    def onInsertionModeStarted(self, label_class):
        ...
        self._label_class = str(label_class)
        ...
        inserter = self._inserterfactory.create(
            ...
            self,
            default_properties,
            max_id=self._labeltool.max_id_dict[str(label_class)]
        )
    ...
    def mousePressEvent(self, event):
        ...
        if self._inserter is not None:
            if not self.sceneRect().contains(event.scenePos()) and \
                ...
                return
            # insert mode
            self._inserter.setMaxID(self._labeltool.max_id_dict[self._label_class])
            self._inserter.mousePressEvent(event, self._image_item)
            self._labeltool._mainwindow.view.activateSceneView(self.getIndex())
        else:
            # selection mode
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self._inserter is not None:
            ...
            if self._labeltool.max_id_dict[self._label_class] < self._inserter.max_id:
                self._labeltool.max_id_dict[self._label_class] = self._inserter.max_id
                self._labeltool.updateAnnotations(self._inserter._ann, self._index)
        else:
            # selection mode
    ...
    def onSelectionChanged(self):
        ...
        self._labeltool.treeviewList()[self.getIndex()].setSelectedItems(model_items)
    ...
    def editSelectedItems(self):
        ...
        if self._inserter is None or len(scene_items) > 0:
            ...
            if self._labeltool.propertyeditor() != None:
