...
from PyQt5.QtCore import QModelIndex, QAbstractItemModel, Qt, pyqtSignal, QVariant
...
class KeyValueModelItem(ModelItem, MutableMapping):
    ...
    def __setitem__(self, key, value, signalModel=True):
        if type(value) == tuple or type(value) == list:
            if len(value) == 1 and type(value[0]) == float:
                value = value[0]
...
class ImageModelItem(ModelItem):
    ...
    def updateAnnotations(self, id_orig, id_new):
        for ann in self.annotations():
            if ann['ID'] == id_orig:
                ann['ID'] = id_new
...
class MultiAnnotationModel(QAbstractItemModel):
    # signals
...
class AnnotationModelBak(QAbstractItemModel):
    # signals
    dirtyChanged = pyqtSignal(bool, name='dirtyChanged')

    def __init__(self, annotation_list, parent=None):
        QAbstractItemModel.__init__(self, parent)

        start = time.time()
        self._annotation_list = annotation_list
        self._dirty = False
        self._root = RootModelItem(self, annotation_list)
        diff = time.time() - start
        LOG.info("Created AnnotationModel in %.2fs" % (diff,))
...
class AnnotationTreeViewBak(QTreeView):
    selectedItemsChanged = pyqtSignal(object)

    def __init__(self, parent=None):
        # super(AnnotationListTreeView, self).__init__(parent)
...
class RootModelItemBak(ModelItem):
    def __init__(self, model, files):
        ModelItem.__init__(self)
        self._model = model
        self._toload = []
    ...
    def appendChild(self, item, signalModel=True):
        if isinstance(item, FileModelItem):
            ...
            raise TypeError("Only FileModelItems can be attached to RootModelItemX")
